/* This is the basic linkfollowing script, modified by @plomlompom. */
...
uzbl.follow.reDrawHints = function(elements, len) {
    ...
    this.documents().forEach(function(doc) {
        ...
        if(uzbl.follow.mode == 'returnuri')
          doc.hintdiv.className = "returnuri";
        if(uzbl.follow.mode == 'download')
          doc.hintdiv.className = "download";
...
uzbl.follow.getInterestingElements = function() {
    ...
    if(uzbl.follow.mode == 'newwindow' || uzbl.follow.mode == 'returnuri' || uzbl.follow.mode == 'download')
...
uzbl.follow.elementSelected = function(el) {
    ...
    } else if (uzbl.follow.mode == 'download') {
        var uri = el.src || el.href;
        return "XXXDOWNLOADXXX" + uri
